...
if(!b||!s)root.innerHTML='<div class="empty">Subject not found. <a href="./">Return to catalogue</a>.</div>';
 ...
 const cards=units.map((u,i)=>{
  ...
  const physicsSpecial=code==='2600102B'&&(i===0||i===1||i===3);
  const href=chemSpecial?'./chemistry-unit'+(i+1)+'.html?v=20260920.15':physicsSpecial?'./physics-unit'+(i+1)+'.html?v=20260920.22':'topic.html?branch='+b.id+'&subject='+encodeURIComponent(code)+'&topic='+encodeURIComponent(u);
