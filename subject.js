...
if(!b||!s)root.innerHTML='<div class="empty">Subject not found. <a href="./">Return to catalogue</a>.</div>';
 ...
 const cards=units.map((u,i)=>{
  ...
  const href=chemSpecial?'./chemistry-unit'+(i+1)+'.html?v=20260920.15':physicsSpecial?'./physics-unit'+(i+1)+'.html?v=20260922.26':'topic.html?branch='+b.id+'&subject='+encodeURIComponent(code)+'&topic='+encodeURIComponent(u);
