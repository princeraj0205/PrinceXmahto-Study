...
if(!b||!s)root.innerHTML='<div class="empty">Subject not found. <a href="./">Return to catalogue</a>.</div>';
 ...
 const cards=units.map((u,i)=>{
  const chemSpecial=code==='2600103B'&&(b.id==='elx'||b.id==='electronics')&&(i===0||i===1||i===2||i===3||i===4);
  const physicsSpecial=code==='2600102B'&&(i===0||i===1||i===2||i===3);
  const href=chemSpecial?'./chemistry-unit'+(i+1)+'.html?v=20260920.15':physicsSpecial?'./physics-unit'+(i+1)+'.html?v=20260922.24':'topic.html?branch='+b.id+'&subject='+encodeURIComponent(code)+'&topic='+encodeURIComponent(u);
  const label=chemSpecial?'Open complete Unit '+(i+1)+' deep notes →':physicsSpecial?'Open complete Unit '+(i+1)+' deep notes →':'Open topic notes →';
